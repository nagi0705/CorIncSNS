// server-middleware/sns/instagram.js

require('dotenv').config();
const express = require('express');

const app = express();
app.use(express.json()); // JSON のパース

// Instagram投稿の関数
async function postToSNS(text, mediaUrl) {
  console.log("📢 Instagram 投稿:", { text, mediaUrl });

  if (!mediaUrl) {
    throw new Error("Instagram の投稿には画像または動画が必須です。");
  }

  try {
    // 🔹 仮の成功レスポンス
    return {
      message: "Dummy Instagram post successful",
      postedData: { text, mediaUrl }
    };
  } catch (error) {
    console.error('Instagram posting error:', error);
    throw new Error("Instagram の投稿に失敗しました");
  }
}

// 既存のエンドポイント（変更なし）
app.post('/api/post-to-instagram', async (req, res) => {
  const { text, mediaUrl } = req.body;

  try {
    const response = await postToSNS(text, mediaUrl);
    return res.json(response);
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
});

module.exports = { app, postToSNS };